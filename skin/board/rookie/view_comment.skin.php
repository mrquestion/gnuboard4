...
<? if ($cwin==1) { // 새창으로 띄우는 경우 ?><table width=100% cellpadding=10><tr><td><?}?>
...
<tr>
    ...
    <td width="100%">
        ...
        <textarea id='save_comment_<?=$comment_id?>' style='display:none;'><?=get_text($list[$i][wr_content], 0); // 수정하기 위해 코멘트를 저장 ?></textarea></td>
</tr>
</table>
<? } ?>
...
<? if ($is_comment_write) { ?>
<!-- 코멘트 입력 -->
<div align=right style='width:<?=$width?>'><a href="javascript:comment_box('', 'c');">코멘트 입력</a></div>
...
<tr>
    ...
    <td width=70 align=center><input type="submit" value="확 인" border=0 accesskey='s'></td></tr>
</table>
</form>
</div>
...
<? if($cwin==1) { ?></td><tr></table><p align=center><a href="javascript:window.close();">창닫기</a><br><br><?}?>
