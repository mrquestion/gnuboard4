...
function bad120422($matches)
{
    ...
    if (preg_match("#script#i", $code)) {
        return "OBJECT ÅÂ±×¿¡ ½ºÅ©¸³Æ®´Â »ç¿ë ºÒ°¡ÇÕ´Ï´Ù.";
    } else if (preg_match("#base64#i", $code)) {
        return "OBJECT ÅÂ±×¿¡ BASE64´Â »ç¿ë ºÒ°¡ÇÕ´Ï´Ù.";
    }
    return $matches[0];
}
...
function conv_content($content, $html)
{
    ...
    if ($html)
    {
        ...
        if ($board[bo_disable_tags])
        {
            ...
            // ÅÂ±×¿¡¸¸ Àû¿ëÇÏ´ø°ÍÀ» ¼Ó¼º(ÇÁ·ÎÆÛÆ¼)¿¡µµ Àû¿ëÇÏµµ·Ï ¼öÁ¤
            $source[] = "/([\<]?)([\/]?)($board[bo_disable_tags])/i";
            $target[] = "$1$2$3-x";
            //$source[] = "/^/";
            //$target[] = "<b>ÀÌ ÆäÀÌÁö´Â »ç¿ë±ÝÁö ÅÂ±× »ç¿ëÀ¸·Î ÀÎÇÏ¿© Á¤»ó Ãâ·ÂµÇÁö ¾ÊÀ» ¼ö ÀÖ½À´Ï´Ù.</b><p>";
        }
        */
        ...
        // ÀÌ·± °æ¿ì¸¦ ¹æÁöÇÔ <IMG STYLE="xss:expr/*XSS*/ession(alert('XSS'))">
        //$content = preg_replace("#\/\*.*\*\/#iU", "", $content); // ÀÌ ÄÚµå¸¦ À§·Î ¿Ã¸³´Ï´Ù.
